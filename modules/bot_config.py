...
class BotConfig(commands.Cog):
    ...
    async def moderators(self, ctx):
        await ctx.reply(embed=embed.error("You need to specify a sub command", "`add` or `remove`"),
    ...
    async def _add(self, ctx, target: Union[discord.Member, discord.Role]):
        if isinstance(target, discord.Role):
            ...
            return await ctx.reply(embed=embed.success(ctx.guild.id, f"**@{target.name}** is now a moderator role"),
                                   mention_author=False)
        if str(target.id) in db.get(f"servers.{ctx.guild.id}.moderators.users", []):
            return await ctx.reply(embed=embed.error(ctx.guild.id, f"**{target}** is already a moderator"),
                                   mention_author=False)
    ...
    async def _remove(self, ctx, target: Union[discord.Member, discord.Role]):
        if isinstance(target, discord.Role):
            ...
            return await ctx.reply(embed=embed.success(ctx.guild.id, f"**@{target.name}** is now not a moderator role"),
                                   mention_author=False)
        elif str(target.id) not in db.get(f"servers.{ctx.guild.id}.moderators.users", []):
            return await ctx.reply(embed=embed.error(ctx.guild.id, f"**{target}** is already not a moderator"),
                                   mention_author=False)
        db.pull(f"servers.{ctx.guild.id}.moderators.users", str(target.id))
        await ctx.reply(embed=embed.success(ctx.guild.id, f"**{target}** is now not a moderator"),
                        mention_author=False)
