...
class ClutterContext(commands.Context):
    async def reply_embed(
        self, asset_type: str, title: str | None = None, /, description: str | None = None, *, mention: bool = False, view: discord.ui.View | None = None
    ) -> discord.Message:
        ...
        """
        return await self.reply(embed=self.bot.embed(asset_type, title, description=description), mention_author=mention, view=view)
    ...
    async def maybe_dm_embed(
        ...
        try:
            return (
                await self.author.send(embed=self.bot.embed(asset_type, title, description=description)),
                True,
            )
