...
class Misc(commands.Cog, name="MODULES.MISCELLANIOUS.NAME", description="MODULES.MISCELLANIOUS.DESCRIPTION"):
    def __init__(self, bot: Clutter):
        ...
        bot.help_command = ClutterHelpCommand(verify_checks=False, brief="COMMANDS.HELP.BRIEF",
    ...
    async def ping(self, ctx: ClutterContext):
        ...
        await message.edit(embed=self.bot.embed.info(self.bot.i18n(ctx, "COMMANDS.PING.RESPONSE.TITLE"),
    ...
    async def invite(self, ctx: ClutterContext):
        ...
        view.add_item(discord.ui.Button(label=self.bot.i18n("COMMANDS.INVITE.BUTTONS.BOT_INVITE"), url=self.bot.invite_url))
        view.add_item(
            discord.ui.Button(label=self.bot.i18n("COMMANDS.INVITE.BUTTONS.DISCORD_INVITE"), url=self.bot.discord_invite))
