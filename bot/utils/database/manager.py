...
class MongoManager:
    ...
    async def get(self, path: str, /, *, default: Any = None) -> Any:
        ...
        if path:
            return find_in_dict(
                await collection.find_one({"_id": _id}, {"_id": 0, ".".join(path): 1}), path, default=default  # type: ignore
            )
    ...
    async def pull(self, path: str, value: Any, /) -> bool:
        ...
        if value in find_in_dict(await collection.find_one({"_id": _id}, {".".join(path): 1}), path, default=[]):  # type: ignore
